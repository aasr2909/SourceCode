...
void  swapRows(int a, int b, int noRows, int noCols)
{
    ...
    if (a != b && a >= 0 && a < noRows && b >= 0 && b < noRows)
    {
        ...
        *(rows[b].columnSet) ^= *(rows[a].columnSet);
        *(rows[a].columnSet) ^= *(rows[b].columnSet);
    }
}
...
void determineColumnsInCommon(int firstRow, int lastRow, int *sharedColumnSet, int NoBs)
{
    int i=0;
    int j=0;
    for (i=0; i<NoBs; i++)
...
int containsColumns(int *columnSet, int  noSets, int NoBs)
{
    ...
    for(i=0;i<noSets;i++)
    {
        ...
        {
            for(j=0;j<NoBs;j++)
                /*Si no existe intersección, finaliza*/
                if((Columns[i][j] & columnSet[j]) == 0)
                    break;
        }
    }
    return contains;
...
int selectRows(int  firstRow, int  lastRow, int  level, int  *overlapping, int noRows, int noCols, int NoBs)
{
    ...
    *overlapping = 0;
    int i=0;
    for(i=firstRow; firstRow <= lastRow; i++)
    {
        ...
        case 2:
        case 1:
            swapRows(lastRow, firstRow, noRows, noCols);
            lastRow--;
            break;
...
void  conquer(int **matrix, int firstRow, int  lastRow, int  level, int noSets, int noCols, int noRows, int minCols, int minRows, int NoBs, int Bits, FILE *Output)
{
    int overlapping=0;
    int splitRow=0;
    ...
    if (firstRow >= 0 && lastRow >= firstRow && lastRow < noRows)
        determineColumnsInCommon(firstRow, lastRow, columnIntersection, NoBs);
    ...
    else
    {
        ...
        if (columnCount(Conditions[level + 1], NoBs, Bits) >= minCols && containsColumns(Conditions[level + 1], noSets, NoBs))
        {
            ...
            if (noSelectedRows >= minRows)
                conquer(matrix, firstRow, firstRow + noSelectedRows - 1, level + 1, noSets, noCols, noRows, minCols, minRows, NoBs, Bits, Output);
        }
...
/*Algoritmo Bimax*/
int Bimax(int **matrix, int noCols, int noRows, int minCols, int minRows, char *FileName)
{
    int Bits=0;
    int NoBs=0;
    ...
    conquer(matrix, 0, noRows-1, 0, 0, noCols, noRows, minCols, minRows, NoBs, Bits, output);
    /*Cierra el archivo*/
    fclose(output);
    /*Libera memoria dinámica*/
    Finalize(noRows);
